...
\item[--] The transition between POI is strongly affected by the user's own preference. Li et al. \cite{Li:2017} refers this as a long-term individual preference. For instance, some people go to the gym after work, but some people go home right away after work. 
...
from user $u$, we can find the estimate via multiplying corresponding entries. ~\cite{Koren:2009}
\begin{equation}
	r_{u,l}= q_l^T p_u
\label{eq:MF}
\end{equation}
...
However, there is a still room for improvement. Li et al. \cite{Li:2017} points out that FPMC-LR approach simply captures 
...
To make the system aware of users' time-varying behavioral trends, Li et al. \cite{Li:2017} suggests time-aware FPMC (TA-FPMC) 
model, which is an improvement over FPMC-LR model. Both FPMC and FPMC-LR use a third order tensor to 
...
user-location-location, it forms a fourth order tensor user-time-location-location by adding a time factor. Our transition tensor looks like: 
\begin{equation}
...
\begin{itemize}
\item[--] Matrix for interaction between user and time period.
\item[--] Matrix for interaction between user and current location.
\item[--] Matrix for interaction between user and next location.
\item[--] Matrix for interaction between time period and current location.
\item[--] Matrix for interaction between time period and next location.
\item[--] Matrix for interaction between current location and next location.
\end{itemize}
